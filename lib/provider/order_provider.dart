...
class OrderProvider with ChangeNotifier {
  ...
  Future<void> placeOrder(
    ...
    ApiResponse apiResponse = await orderRepo.placeOrder(
      ...
      couponCode,
      orderNote,
      orderPlaceModel.paymentMethod,
      orderPlaceModel.paymentMethodId,
      orderPlaceModel.paymentImage,
    );
    _isLoading = false;
    if (apiResponse.response != null &&
        apiResponse.response.statusCode == 200) {
  ...
  Future<void> uploadImage(File image, Function callback) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse = await orderRepo.uploadImage(image);
    // _isLoading = false;
    print('uploadImageAPI ${apiResponse.response}');
    if (apiResponse.response != null &&
        apiResponse.response.statusCode == 200) {
      String message = apiResponse.response.data['message'].toString();
      String image = apiResponse.response.data['url'].toString();
      _isLoading = false;
  ...
  Future<void> initPaymethMethodList(BuildContext context) async {
    ApiResponse apiResponse = await orderRepo.getPaymentMethodList();
    if (apiResponse.response != null &&
        apiResponse.response.statusCode == 200) {
      _paymentMethodList = [];
      apiResponse.response.data['data'].forEach(
          (m) => _paymentMethodList.add(PaymentMethodModel.fromJson(m)));
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
    notifyListeners();
