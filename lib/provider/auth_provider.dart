...
class AuthProvider with ChangeNotifier {
  ...
  SellerRegisterModel _sellerRegisterModel;
  SellerRegisterModel get sellerRegisterModel => _sellerRegisterModel;
  ...
  updateVerificationButtonLoading(bool v) {
    _isPhoneNumberVerificationButtonLoading = v;
    notifyListeners();
  }
  ...
  Future<ResponseModel> checkPhoneNumber(
    ...
    if (apiResponse.response.statusCode == 200) {
      print("CheckResponsePhone3 ${apiResponse.response.statusCode}");
      _isLoading = false;
  ...
  Future<ResponseModel> verifyPhone(
      String phone, String tempToken, String verificationId) async {
    _isPhoneNumberVerificationButtonLoading = true;
    _verificationMsg = '';
    notifyListeners();

    ResponseModel responseModel;
    bool verify = await SMSModel()
        .smsVerify(phone, _verificationCode, verificationId, (error) {
      _verificationMsg = error;
    });

    if (verify) {
      ApiResponse apiResponse = await authRepo.verifyPhone(tempToken, phone);
  ...
  Future<ResponseModel> verifyOtp(String phone, String verificationId) async {
    _isPhoneNumberVerificationButtonLoading = true;
    _verificationMsg = '';
    notifyListeners();
    ...
    _isPhoneNumberVerificationButtonLoading = false;
    notifyListeners();
    return ResponseModel(_verificationMsg, verify);
  }

  Future<ResponseModel> resetPassword(String identity, String tempToken,
      String password, String confirmPassword) async {
    _isPhoneNumberVerificationButtonLoading = true;
    _verificationMsg = '';
    notifyListeners();
    ApiResponse apiResponse = await authRepo.resetPassword(
        identity, tempToken, password, confirmPassword);
    _isPhoneNumberVerificationButtonLoading = false;
    notifyListeners();
    ResponseModel responseModel;
    if (apiResponse.response != null &&
  ...
  Future sellerRegistration(
      SellerRegisterModel sellerRegisterModel, Function callback) async {
    _isLoading = true;
    notifyListeners();

    ApiResponse apiResponse =
        await authRepo.sellerRegistration(sellerRegisterModel);
    _isLoading = false;
  ...
  Future<ResponseModel> forgetPassword(String phoneNumber) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse = await authRepo.forgetPassword(phoneNumber);
    _isLoading = false;
    notifyListeners();
    ResponseModel responseModel;
    if (apiResponse.response != null &&
