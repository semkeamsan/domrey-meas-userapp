...
class AuthRepo {
  ...
  Future<ApiResponse> sellerRegistration(
      SellerRegisterModel sellerRegisterModel) async {
    print('DataDATATATA ${sellerRegisterModel.profileImage}');
    try {
      Response response = await dioClient.post(
        AppConstants.SELLER_REGISTRATION_URI,
        data: sellerRegisterModel.toJson(),
      );
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
  ...
  //   }
  // }
  // Future<ApiResponse> checkPhoneNumber(String phone) async {
  //   print(phone);
  //   try {
  ...
  Future<ApiResponse> verifyPhone(String token, String phone) async {
    try {
      Response response =
          await dioClient.post(AppConstants.VERIFY_TOKEN_URI, data: {
        'phone': phone,
  ...
  Future<ApiResponse> resetPassword(String identity, String tempToken,
