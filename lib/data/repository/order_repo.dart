...
class OrderRepo {
  ...
  Future<ApiResponse> placeOrder(
      ...
      int paymentMethodId,
      String paymentImage) async {
    try {
      final response = await dioClient.get(AppConstants.ORDER_PLACE_URI +
          '?address_id=$addressID&coupon_code=$couponCode&order_note=$orderNote&payment_method=$paymentMethod&payment_method_id=$paymentMethodId&payment_image_url=$paymentImage');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> getPaymentMethodList() async {
    try {
      final response = await dioClient.get(AppConstants.PAYMENT_METHOD_URI);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
